
function Fullstack() {
  return (
    <div className="container">
      <div className="row">

        <h2 className="course">Full Stack Development</h2>

        <div className="col-lg-4">
          <div class="card" style={{ width: "25rem" }}>
            <img src="https://static.guvi.in/webinar/fsdcareerguidance.png" class="card-img-top" alt="..." />
            <div class="card-body">
              <p class="card-text">Full Stack Development Career Guidance</p>
            </div>
          </div>
        </div>

        <div className="col-lg-4">
          <div class="card" style={{ width: "25rem" }}>
            <img src="https://static.guvi.in/webinar/becomefsd.png" class="card-img-top" alt="..." />
            <div class="card-body">
              <p class="card-text">Become a Full Stack Developer in 90 Days</p>
            </div>
          </div>
        </div>

        <div className="col-lg-4">
          <div class="card" style={{ width: "25rem" }}>
            <img src="https://static.guvi.in/webinar/datasciencevsfsd.png" class="card-img-top" alt="..." />
            <div class="card-body">
              <p class="card-text">Data Science vs Full Stack Developer</p>
            </div>
          </div>
        </div>

        <div className="col-lg-4">
          <div class="card" style={{ width: "25rem" }}>
            <img src="https://static.guvi.in/webinar/howtobecomemernstackdeveloper.png" class="card-img-top" alt="..." />
            <div class="card-body">
              <p class="card-text">How to become a MERN Stack Developer</p>
            </div>
          </div>
        </div>

        <div className="col-lg-4">
          <div class="card" style={{ width: "25rem" }}>
            <img src="https://static.guvi.in/webinar/programmingroadmap.png" class="card-img-top" alt="..." />
            <div class="card-body">
              <p class="card-text">Programming road map</p>
            </div>
          </div>
        </div>

        <div className="col-lg-4">
          <div class="card" style={{ width: "25rem" }}>
            <img src="https://static.guvi.in/webinar/fsdforaiandbuildchatbot.png" class="card-img-top" alt="..." />
            <div class="card-body">
              <p class="card-text"> FSD For AI Use cases & Building Chat Bots</p>
            </div>
          </div>
        </div>

      </div>
    </div>
  )
}

export default Fullstack